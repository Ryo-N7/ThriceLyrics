...
Let's get started!
Using the `wordToken2` data set that we created in **Part 2** we will filter out any numbers in our data set and then use a `left_join()` function to:
...
```
With the size of the word dictated by the size and central position in the cloud, this is another cool way to visualize the most common words. As seen before for positive words, **Love** appears the most whereas for negative, **Fall** appears most frequently.
