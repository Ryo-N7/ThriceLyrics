...
tidy_lyrics %>% 
  ...
  mutate(sentiment_ratio = (positive - negative) / (positive + negative + neutral)) # include neutral?
...
emotion_box %>% 
  select(-album) %>% 
  boxplot2(col = cols, lty = 1, top = TRUE, textcolor = "red", shrink = 0.7,
...
ggplot(emotions_lyrics_nrc, aes(x = album, y = percent/100, color = sentiment, group = sentiment)) + 
  geom_line(size = 1) + 
  geom_point(size = 3) +
