...
_Major/Minor_ and _Vheissu_ are the longest albums, both totalling up to a bit over 49 mins!
...
Besides grouping with `group_by()` and summarizing with `summarize()`, there are other ways to filter our data. For example, let's say we want to see the total duration of _The Alchemy Index_ (*Fire*, _Water_, _Earth_, and _Air_) then we could use the `grepl()` function to search for all albums with the term "Index" in it:
